...
if(listado!=null){
	for(Salon s:listado){
		%>
	<div class="btn-group btn-group-lg w-100 my-2" role="group">
		<a class="btn btn-outline-light disabled"> <%=s.getIdsalon() %> </a>
		<a class="btn btn-outline-light w-75"
			href="ControllerProfesor?value=Notas&salon=<%=s.getIdsalon()%>&id=<%= p.getIdprofesor()%>">
			<%=s.getCurso().getNombre() %> 
		</a>
		<a class="btn btn-outline-light disabled"><%=s.getTurno() %></a>	
	</div>
		<%} 
	} %>
